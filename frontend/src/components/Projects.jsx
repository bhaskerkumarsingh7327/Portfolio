        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: '1.5rem' }}>
          {projects.map((p, i) => (
            <div key={p.id} onMouseEnter={() => setHov(p.id)} onMouseLeave={() => setHov(null)}
              style={{ background: hov === p.id ? 'rgba(0,212,255,0.04)' : '#0a1520', border: `1px solid ${hov === p.id ? p.color + '60' : 'rgba(0,212,255,0.1)'}`, padding: '1.8rem', position: 'relative', cursor: 'default', transition: 'all 0.35s cubic-bezier(0.4,0,0.2,1)', transform: hov === p.id ? 'translateY(-6px)' : 'translateY(0)', boxShadow: hov === p.id ? `0 12px 40px ${p.color}15` : 'none', opacity: vis ? 1 : 0, transitionDelay: `${i * 0.1}s` }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: hov === p.id ? `linear-gradient(to right,transparent,${p.color},transparent)` : 'transparent', transition: 'all 0.3s' }} />
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                ...
                <span style={{ padding: '0.2rem 0.6rem', fontSize: '0.65rem', fontFamily: "'Share Tech Mono',monospace", letterSpacing: 2, background: `${statusColor[p.status]}15`, color: statusColor[p.status], border: `1px solid ${statusColor[p.status]}40` }}>{p.status}</span>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '1.5rem' }}>
                {p.tech.map(t => (
                  <span key={t} style={{ padding: '0.15rem 0.55rem', fontSize: '0.68rem', fontFamily: "'Share Tech Mono',monospace", background: `${p.color}0d`, color: `${p.color}cc`, border: `1px solid ${p.color}25`, letterSpacing: 1 }}>{t}</span>
                ))}
              </div>

              <div style={{ display: 'flex', gap: '1rem' }}>
                <a href={p.link} style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: '0.7rem', color: p.color, textDecoration: 'none', letterSpacing: 2, transition: 'opacity 0.2s' }} onMouseEnter={e => e.currentTarget.style.opacity = '0.7'} onMouseLeave={e => e.currentTarget.style.opacity = '1'}>↗ LIVE</a>
                <a href={p.github} style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: '0.7rem', color: 'rgba(200,232,240,0.4)', textDecoration: 'none', letterSpacing: 2, transition: 'color 0.2s' }} onMouseEnter={e => e.currentTarget.style.color = '#fff'} onMouseLeave={e => e.currentTarget.style.color = 'rgba(200,232,240,0.4)'}>⌥ CODE</a>
              </div>
            </div>
          ))}
        </div>
